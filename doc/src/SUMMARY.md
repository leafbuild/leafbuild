# Summary

- [Introduction](introduction.md)
- [Quick Start](quick_start.md)
- [Supported Languages](supported_languages.md)
- [Syntax](syntax.md)
- [Project Model](project_model.md)

# Developer resources
- [Setting up for development](dev/setup.md)
- [Syntax reference](dev/syntax_ref.md)
- [Terminology](dev/terminology.md)
- [Architecture](dev/architecture-overview.md)
  - [`leafbuild-syntax`](dev/leafbuild-syntax.md)
  - [`leafbuild-interpreter`](dev/leafbuild-interpreter.md)
  - [`leafbuild-ninja-be`](dev/leafbuild-ninja-be.md)
  - [`leafbuild-make-be`](dev/leafbuild-make-be.md)
  - [`leafbuild-ml`](dev/leafbuild-ml.md)
  - [`leafbuild-cmakeml`](dev/leafbuild-cmakeml.md)
  - [`leafbuild-mesonml`](dev/leafbuild-mesonml.md)
- [The Documentation](dev/documentation.md)
  - [Highlighter](dev/highlighter.md)
- [Root makefile recipes](dev/root_makefile_recipes.md)
- [Validation checks](validation_checks.md)

# Others
- [Motivation](motivation.md)
- [Roadmap](roadmap.md)
