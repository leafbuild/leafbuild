...
impl LeafDiagnosticTrait for InvalidIndexError {
    fn get_diagnostic(self, ctx: &DiagnosticsCtx) -> LeafDiagnostic {
        LeafDiagnostic::error()
            ...
            .with_labels(vec![
                LeafLabel::primary(ctx.get_current_file(), self.base_expr.loc)
                    .with_message(self.base_expr.type_),
                LeafLabel::secondary(ctx.get_current_file(), self.index_expr.loc)
                    .with_message(self.index_expr.type_),
            ])
    }
